...
const applySpecialistRules = async () => {
  ...
  if (specialist === '30') {
    newStrats = await newStrats.filter(
      (ns) => ns.category === 'Eagle' && ns.displayName !== 'Eagle Strafing Run',
    );
    return;
  }
  ...
  if (specialist === '31') {
    newStrats = await newStrats.filter(
      (ns) => ns.category === 'Orbital' && ns.displayName !== 'Orbital Precision Strike',
    );
    return;
  }
  ...
  if (specialist === '32') {
    ...
    newStrats = await newStrats.filter((ns) => mineStrats.includes(ns.displayName));
    return;
  }
  ...
  if (specialist === '33') {
    newStrats = await newStrats.filter(
      (ns) => ns.tags.includes('Sentry') && ns.displayName !== 'Machine Gun Sentry',
    );
    return;
  }
  ...
  if (specialist === '38') {
    newStrats = await newStrats.filter(
      (ns) =>
        ns.category === 'Eagle' ||
        (ns.tags.includes('Vehicles') && ns.displayName !== 'Breakthrough Exosuit'),
    );
    return;
  }
  ...
  if (specialist === '40') {
    newStrats = await newStrats.filter(
      (ns) => ns.antitank === true && ns.displayName !== 'Anti-Tank Emplacement',
    );
    return;
  }
