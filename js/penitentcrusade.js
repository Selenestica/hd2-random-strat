...
const showAllRewardsModal = document.getElementById("showAllRewardsModal");
const showAllRewardsModalBody = document.getElementById(
  "showAllRewardsModalBody",
);
...
const handleToggleAllWarbonds = (e) => {
  ...
  allWarbondCheckboxes.forEach((checkbox) => {
    // Skip the Helldivers Mobilize checkbox since it's disabled
...
const updateToggleAllButton = () => {
  ...
  const allWarbondCheckboxes = document.querySelectorAll(".warbondCheckboxes");
  // Filter out the disabled Helldivers Mobilize checkbox
...
const genAllRewardsModalContent = async () => {
  const data = await localStorage.getItem("penitentCrusadeSaveData");
  if (!data) {
    return;
  }
  showAllRewardsModalBody.innerHTML = "";
  const game = await getCurrentGame("penitentCrusadeSaveData");
  const chosenRewards = game.acquiredItems;
  for (let i = 0; i < chosenRewards.length; i++) {
    const vals = await getItemMetaData(chosenRewards[i]);
    const card = await generateItemCard(
      chosenRewards[i],
      false,
      vals.imgDir,
      null,
    ...
    showAllRewardsModalBody.innerHTML += card;
  }
};
...
const applySpecialistRules = async () => {
  ...
  // only Sentries for Combat Engineer
  if (specialist === "33") {
    newStrats = await newStrats.filter(
      (ns) =>
        ns.tags.includes("Sentry") && ns.displayName !== "Machine Gun Sentry",
    );
    return;
  }

  // only Vehicles/Mechs and Eagles for The Pilot
  if (specialist === "38") {
    newStrats = await newStrats.filter(
      (ns) =>
        ns.category === "Eagle" ||
  ...
  if (specialist === "44") {
    newStrats = await newStrats.filter(
      (ns) =>
        ns.tags.includes("expendable") && ns.displayName !== "Bullet Storm",
    );
    return;
  }
...
const uploadSaveData = async () => {
  ...
  if (penitentCrusadeSaveData) {
    ...
    for (let i = 0; i < missingWarbondCodes.length; i++) {
      const checkbox = document.getElementById(missingWarbondCodes[i]);
      if (checkbox && checkbox.id !== "warbond3") {
