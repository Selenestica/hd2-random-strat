...
const deleteSavedGameData = async () => {
  ...
  if (saveIndex !== undefined) {
    ...
    const theGauntletSaveData = JSON.parse(
      localStorage.getItem("theGauntletSaveData"),
    );
    let tempArray = [...theGauntletSaveData.savedGames];
    tempArray.splice(saveIndex, 1);
    let tempObj = {
      ...theGauntletSaveData,
      savedGames: tempArray,
    };
    localStorage.setItem("theGauntletSaveData", JSON.stringify(tempObj));
  }
  clearSaveDataManagementModal();
};
...
const uploadSaveFile = async () => {
  ...
  if (!theGauntletSaveData) {
    ...
    await localStorage.setItem("theGauntletSaveData", JSON.stringify(obj));
    applySavedGameData(true);
    uploadedSaveFile = null;
    saveDataUploadInput.value = "";
    return;
  }
  let newData = JSON.parse(theGauntletSaveData);
